...
This project contains source code and supporting files for SugarLive Lambda functions that you can deploy with the AWS Serverless Application Model (AWS SAM) command line interface (CLI). It includes the following files and folders:

- `src` - Code for the application's Lambda functions.
- `events` - Invocation events that you can use to invoke the functions for testing locally.
...
```bash
my-application$ sam local invoke helloFromLambdaFunction -n env.json -e ./events/hello-from-lambda.json 
```
